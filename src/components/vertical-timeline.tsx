...
function VerticalTimeline({ children }: { children: React.ReactNode }) {
	return (
		<div className='relative pt-7 w-full flex flex-col gap-[2.5rem] overflow-hidden'>
			<div className='absolute w-1 h-full rounded-full bg-white/60 dark:bg-slate-700/25 left-10 sm:left-1/2 -translate-x-1/2 -translate-y-7'></div>
			{children}
		</div>
	);
}
...
function VerticalTimelineElement({
	...
	return (
		<div
			...
			<motion.div
				className='absolute rounded-full w-14 aspect-square bg-white/60 left-10 sm:left-1/2 flex justify-center items-center dark:bg-slate-700/25 dark:backdrop-blur-[0.5rem]'
			...
			<div
				...
				<motion.div
					className='relative px-5 py-3 rounded-lg bg-white/60 dark:bg-slate-700/25 text-wrap'
